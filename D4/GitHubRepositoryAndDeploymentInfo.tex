...
\begin{enumerate}
    ...
    \item Creare file \inlinecode{.env.local} e se si vuole fare il testing bisogna anche creare \inlinecode{.env.test.local}.
    \item Inserire
          \begin{itemize}
              \item \inlinecode{MONGODB_URI = 'mongodb+srv://"user":"pass"@"cluste".mongodb.net/PlanIt?retryWrites=true&w=majority'}
              \item \inlinecode{AUTH0_SECRET= 'inserire auth0 secret'}
              \item \inlinecode{AUTH0_BASE_URL='http://localhost:3000'}
              \item \inlinecode{AUTH0_ISSUER_BASE_URL='inserire base_url'}
              \item \inlinecode{AUTH0_CLIENT_ID='inserire clienId'}
              \item \inlinecode{AUTH0_CLIENT_SECRET='inserire client secret'}
          \end{itemize}
    \item Eseguire \inlinecode{npm run dev} o \inlinecode{npm run build} e \inlinecode{npm run test}
\end{enumerate}
