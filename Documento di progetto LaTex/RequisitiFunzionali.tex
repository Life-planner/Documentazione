...
\begin{listaPersonale}{RF}
	...
	\begin{listaPersonale2}{RF}
		\elemento[METODI AUTENTICAZIONE]{rf:1.1} L'autenticazione può essere fatta con email e password o con servizi di terze parti, più nello specifico con Google o con eventuali servizi di terze parti (\ref{ob:1}).
	\end{listaPersonale2}
	...
	\begin{listaPersonale2}{}
		\elemento{rf:5.1} d'imporre restrizioni, ovvero definire ora e giorno dell'attività da svolgere;
		...
		\elemento{rf:5.5} l'utente avrà la possibilità d'impostare le ore di sonno giorno per giorno oppure applicare le stesse ore per più giorni, chiamate "Abitudini";
		...
		\elemento {rf:5.8} di poter aggiungere l'evento ad un raggruppamento di altre attività; un raggruppamento è un insieme di attività che non hanno restrizioni di orario;
		\elemento {rf:5.9} impostare la difficoltà, ovvero definire quanto l'evento sia difficile da completare.
	\end{listaPersonale2}


	\elemento[IMPOSTAZIONE IMPEGNO]{rf:6} Una volta compilato il pop-up di aggiunta impegno, il sistema deve inserire automaticamente l'impegno nel calendario (\ref{ob:3}) seguendo le restrizioni, difficoltà, priorità e gli altri campi inseriti dall'utente a tempo di compilazione dell'evento (\ref{rf:5}).
