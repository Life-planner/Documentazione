...
Nel presente capitolo viene discusso il contesto di funzionamento del sistema (ACI), fornendo una descrizione testuale e una rappresentazione grafica basata su Context Diagram (DC).
...
\begin{listaPersonale}[ACI]{}
    ...
    \elemento[Utente non autenticato:]{aci:UtenteNonAutenticato}
    ...
    \elemento[Utente autenticato standard:]{aci:UtenteAutenticato}
    ...
    \elemento[Utente autenticato premium:]{aci:UtentePremium}
    ...
    \elemento[Google Calendar:]{aci:GoogleCalendar}
    ...
    \elemento[Google Maps:]{aci:GoogleMaps}
    ...
    \elemento[OpenStreetMap:]{aci:OpenStreetMap}
    Uno dei possibili metodi utilizzato per indicare il luogo dove si svolge un evento, come specificato in \prettyref{D1-rf:LuogoEvento}

    \elemento[PayPal:]{aci:PayPal}
    ...
    \elemento[Stripe:]{aci:Stripe}
    Uno dei possibili metodi per effettuare il pagamento della sottoscrizione all'account premium, la cui possibilità è descritta in \prettyref{D1-rf:PagamentoUtentePremium}.

    \elemento[Auth0:]{aci:Auth0}
    ...
    \elemento[Cloudflare:]{aci:Cloudflare}
    Viene utilizzato per essere protetti da attacchi di tipo DDOS e per gestire le richieste DNS \prettyref{D1-rnf:Sicurezza}.

    \color{red}
    \elemento[MongoDB:]{aci:MongoDB}
    Viene utilizzato come database per memorizzare le informazioni relative a utenti, calendari ed eventi. A che RF lo colleghiamo?
    \color{black}

    \elemento[Sistema di posta elettronica:]{aci:PostaElettronica}
...
\begin{listaPersonale}[DC]{}
    \elemento[Utente non autenticato]{dc:UtenteNonAutenticato}
    ...
    \elemento[Utente autenticato standard]{dc:UtenteAutenticato}
    ...
    \elemento[Utente autenticato premium]{dc:UtentePremium}
    Un utente autenticato premium deve poter modificare le proprie informazioni relative al pagamento \prettyref{D1-rf:PagamentoUtentePremium}.

    \elemento[Google Calendar]{dc:GoogleCalendar}
    ...
    \elemento[Google Maps/OpenStreetMap]{dc:Mappe}
    ...
    \elemento[PayPal/Stripe]{dc:Pagamento}
    Per poter effettuare il pagamento, \prettyref{D1-rf:PagamentoUtentePremium}, sarà inviata direttamente una richiesta di pagamento ai servizi esterni Stripe e PayPal, tramite cui l'utente standard pagherà. Una volta completato il pagamento il sistema riceve l'esito e abilita le funzionalità premium.

    \elemento[Iubenda]{dc:PrivacyCookie}
    ...
    \elemento[MongoDB]{dc:Database}
    Per poter rispettare il \prettyref{D1-rf:ImpostazioniAccount}, il sistema invierà una richiesta di email di recupero password a un sistema di posta elettronica che provvederà all'invio dell' email di recupero all'utente. \color{red}da chiedere\color{black}

    \elemento[Auth0:]{dc:Auth0}
    ...
    \elemento[Cloudflare]{dc:Cloudflare}
